// Based on https://github.com/Strajk/setup/blob/master/programs/prevent-spotlight-from-indexing-node-modules.js
...
// Customize
// ===
const DIR = `${HOME}/projects`; // TODO: Maybe take from args?
const PASSWORD = process.env.SUDO_PASSWORD || 'FILLTHISIN'; // TODO: Better security, see note about sudo-prompt
...
const execSyncSudo = (input, options = {}) =>
...
const existing = cleanArray(
  execSyncSudo(`${BUDDY} -c "Print :Exclusions" ${PLIST} | sed -e 1d -e '$d'`),
); // TODO: Nicer way of getting pure array of results
